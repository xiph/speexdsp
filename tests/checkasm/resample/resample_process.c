...
void checkasm_check_resample_process(void)
{
    ...
    CHECKASM_ALIGN(float fout_ref[OUT_LEN]);
    CHECKASM_ALIGN(float fout_new[OUT_LEN]);
    CHECKASM_ALIGN(spx_int16_t iin[IN_BUF]);
    CHECKASM_ALIGN(spx_int16_t iout_ref[OUT_LEN]);
    CHECKASM_ALIGN(spx_int16_t iout_new[OUT_LEN]);
